...
class MusicEngine {
  ...
  let nextBarLogicTick = 15 // when we run the logic to schedule the next bar loop, advance the block counter etc..
  ...
  func initializeEngine() {
    let allAudioPlayers = loopPlayers.map { $0.audioPlayer }
          ...
          if current16thNoteInOneBar == musicEngine.nextBarLogicTick {
            do {
              ...
              for loopPlayer in musicEngine.loopPlayers where loopPlayer.loopPlaying {
                if !loopPlayer.audioPlayer.isPlaying {
                  ...
                  loopPlayer.audioPlayer.play(at: avTime)
                }
              }
            } catch let error {
